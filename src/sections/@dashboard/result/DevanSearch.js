...
const DevanSearch = ({ selectedOrders, thisMonthSubmissions, onCheckboxChange, checkboxState, searchQuery }) => {
  ...
  useEffect(() => {
    const fetchThisMonthSubmissions = async () => {
      try {
        const response = await axios.get(`http://localhost:4000/api/submissions/search-by-postal-code-Devan?query=${searchQuery}`);
